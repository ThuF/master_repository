...
exports.createDelchev_books = function() {
    ...
    try {
        ...
        var i = 0;
        var id = $.getDatabaseUtils().getNext('DELCHEV_BOOKS_BOOKID');
...
exports.readDelchev_booksEntity = function(id) {
    var connection = $.getDatasource().getConnection();
    try {
        var result;
        var statement = connection.prepareStatement("SELECT * FROM DELCHEV_BOOKS WHERE " + exports.pkToSQL());
...
exports.readDelchev_booksList = function(limit, offset, sort, desc) {
    var connection = $.getDatasource().getConnection();
    try {
        var result = [];
        var sql = "SELECT ";
...
// update entity by id
exports.updateDelchev_books = function() {
    var input = ioLib.read($.getRequest().getInputStream());
    var responseBody = JSON.parse(input);
    var connection = $.getDatasource().getConnection();
    try {
        var sql = "UPDATE DELCHEV_BOOKS SET ";
        sql += "BOOKISBN = ?";
        sql += ",";
        sql += "BOOKTITLE = ?";
        sql += ",";
...
// delete entity
exports.deleteDelchev_books = function(id) {
    var connection = $.getDatasource().getConnection();
    try {
        var statement = connection.prepareStatement("DELETE FROM DELCHEV_BOOKS WHERE " + exports.pkToSQL());
        statement.setString(1, id);
        statement.executeUpdate();
        $.getResponse().getWriter().println(id);
    } catch(e){
...
exports.countDelchev_books = function() {
    var count = 0;
    var connection = $.getDatasource().getConnection();
    try {
        var statement = connection.createStatement();
        var rs = statement.executeQuery('SELECT COUNT(*) FROM DELCHEV_BOOKS');
        if (rs.next()) {
            count = rs.getInt(1);
        }
    } catch(e){
...
exports.metadataDelchev_books = function() {
	var entityMetadata = {
		name: 'delchev_books',
		type: 'object',
		properties: []
...
function ExceptionDelchev_books(message) {
   this.message = message;
   this.name = "ExceptionDelchev_books";
}
...
exports.getPrimaryKeys = function() {
    ...
    result[i++] = 'BOOKID';
    if (result === 0) {
        throw new ExceptionDelchev_books("There is no primary key");
    } else if(result.length > 1) {
        throw new ExceptionDelchev_books("More than one Primary Key is not supported.");
    }
    return result;
};
